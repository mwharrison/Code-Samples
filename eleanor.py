...
def sms(to, message):
    ...
    cur.execute("""INSERT INTO eleanor_sms(`sent`, `to`, `message`, `response`) VALUES (%s ,%s, %s, %s)""",
                (current_mysql_time, to, message, sms))
    db.commit()
...
nest2 = subprocess.check_output("python nest.py --user='mikey_con@me.com' --password='[PASSWORD]' --index=1 show",
                                shell=True)
nest3 = subprocess.check_output("python nest.py --user='mikey_con@me.com' --password='[PASSWORD]' --index=0 show",
                                shell=True)
...
temp2_update = "The temperature on floor two is %.1f degrees Celsius. The humidity is at %i percent" % (
    temp2, humidity2)
temp3_update = "The temperature on floor three is %.1f degrees Celsius and the humidity is at %i percent" % (
    temp3, humidity3)
...
def weather():
    weatherC = subprocess.check_output(
        "curl --silent 'http://xml.weather.yahoo.com/forecastrss?p=USTX0617&u=c' | grep -E '(Current Conditions:|C<BR)' | sed -e 's/Current Conditions://' -e 's/<br \/>//' -e 's/<b>//' -e 's/<\/b>//' -e 's/<BR \/>//' -e 's///' -e 's/<\/description>//'",
        shell=True)
    weatherF = subprocess.check_output(
        "curl --silent 'http://xml.weather.yahoo.com/forecastrss?p=USTX0617&u=f' | grep -E '(Current Conditions:|F<BR)' | sed -e 's/Current Conditions://' -e 's/<br \/>//' -e 's/<b>//' -e 's/<\/b>//' -e 's/<BR \/>//' -e 's///' -e 's/<\/description>//'",
        shell=True)
...
def main():
    ...
    if (cmd == 'e'):
        ...
        if (do[0] == 'download'):
            ...
            subprocess.check_output("mpg123 -q 'http://www.televisiontunes.com/themesongs/Jurassic%20Park.mp3'",
                                    shell=True)
